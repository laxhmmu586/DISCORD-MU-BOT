...
test('updating one CBS case keeps the complete case list visible', () => {
  assert.match(page, /window\._selectedCbsRow = 0;\s*caseSearch\.value = '';\s*window\._expandedCbsCases = window\._expandedCbsCases \|\| new Set\(\);\s*window\._expandedCbsCases\.add\(`row-\$\{rowNumber\}`\);\s*await loadCases\(\);\s*await loadUnresolvedBaggage\(\)/);
...
test('CBS removes requested helper descriptions', () => {
  [
    ...
    'Outbound bags registered as Not Load Bags remain here until resolved.',
    'On-hand cases completed by Create Rush, Shipping, Passenger collected, or Case Close are archived here.',
    'Completed Not Load Bags cases are archived here.'
...
test('CBS updates clear stale filters and reload both case data sources', () => {
  ...
  assert.match(passengerUpdate, /window\._selectedCbsRow = 0;\s*caseSearch\.value = '';[\s\S]*await loadCases\(\);\s*await loadUnresolvedBaggage\(\)/);
  const onHandUpdate = page.match(/openCasesCard\?\.addEventListener\('submit'[\s\S]*?addBaggageSearch\?\.addEventListener/)?.[0] || '';
  assert.match(onHandUpdate, /window\._selectedCbsRow = 0;\s*caseSearch\.value = '';\s*await loadCases\(\);\s*await loadUnresolvedBaggage\(\);\s*await loadWorldTracerCases\(\)/);
});
...
test('CBS loads all case lists sequentially to avoid concurrent Sheets failures', () => {
  assert.match(page, /async function loadInitialCaseData\(\) \{[\s\S]*await loadCases\(\);\s*await loadUnresolvedBaggage\(\);\s*await loadMissingReports\(\);\s*await syncMissingReports\(\);/);
  assert.match(page, /showSection\(initialSection\);\s*loadInitialCaseData\(\);/);
  assert.doesNotMatch(page, /showSection\(initialSection\);\s*loadCases\(\);\s*loadMissingReports\(\);/);
});

test('CBS case refresh reads Google Sheets and does not restart the page', () => {
  assert.match(drive, /async function getCbsCases\(\) \{[\s\S]*getCbsSheetRows/);
  assert.match(page, /await loadCases\(\);\s*await loadUnresolvedBaggage\(\);\s*await loadMissingReports\(\);/);
...
test('case overview cards omit arrows and remain a three-column mobile grid', () => {
  ...
  assert.match(page, /\.case-overview \{ display:grid; grid-template-columns:repeat\(3,minmax\(0,260px\)\); gap:12px; justify-content:start/);
...
test('On-hand only includes Office bags and excludes gate bags and Co-mail', () => {
  assert.doesNotMatch(page, /id="on-hand-description"|id="bag-room-unload-description"/);
