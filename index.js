...
app.get('/search', async (req, res) => {
  try {
    ...
    // =========================
    // 历史搜索格式: BN/Ticket/Seat/FF + /DDMMM[YY]
    // =========================
    ...
    // =========================
    let log = null;
    if (date) log = await getFlightLogByDate(date);  // Archive
    else log = await getLatestFlightLog();           // Today
    ...
    // =========================
      ...
      if (!pax && date) pax = findPDByFFNumber(q); // PD fallback for history
    }
    // =========================
    // Name search (today only)
    // =========================
    else if (!date) {
      pax = findByName(q);
    }
